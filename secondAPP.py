...
# Coordinates mapping for each pickup ID
    ...
    # Prepare data for clustering (latitude, longitude, and demand as weight)
        ...
        try:
            ...
            if (weights < 0).any():
                raise ValueError("Negative probabilities detected in Predicted Demand.")
            ...
            st.warning(f"Not enough predictions to triangulate a position")
        ...
        def initialize_recent_bookings():
            if not st.session_state.initialized_30_days:
                current_date = datetime.now().date()  # Use today's date as the base date

                base_time = datetime.combine(current_date, time(0, 0))  # Start at midnight
                ...
                for hour in range(24):
                    pickup_time = base_time + timedelta(hours=hour)
        ...
        def evaluate_shuttle(shuttle, new_request):
            ...
            earliest_arrival_time = datetime.now() + timedelta(minutes=total_duration)
        ...
        def find_next_available_time():
            next_available_time = datetime.now() + timedelta(minutes=15)
            ...
            for shuttle_routes in st.session_state.shuttle_routes.values():
                if shuttle_routes:  # Only process if there are existing routes for the shuttle
                    ...
                    shuttle_next_available_time = latest_request_time + timedelta(minutes=15)
        ...
        pickup_time = st.time_input("Choose your desired pickup time", time(13, 0))
        pickup_datetime = datetime.combine(date.today(), pickup_time)
        dropoff_time = pickup_datetime + timedelta(minutes=30)
        ...
        if st.button("Create Booking"):
            ...
            for shuttle in st.session_state.shuttle_routes.keys():
                ...
                if eval_info["is_feasible"]:
                    ...
                    - **Status**: Can accommodate the request at the recommended time.
                    """
